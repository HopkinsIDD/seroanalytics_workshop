...
If this directory is not the one we want, we need to re-set. There are many ways to do this, we will share one way.
...
Now, we can see there are 13 different plates run, with varying numbers of individual samples run (ranging from 22 to 82 per plate). 
...
The proportion of households with female head is thus `r sum(meta_data$sex1 == "Female") / sum(meta_data$sex1 == "Female" | meta_data$sex1 == "Male")`.  
...
#let's make the serostatus variables categorical
...
Now let's cover how to make plots using the `ggplot2` package. While all plots can be made using base R (not using the ggplot2 package), we will make different types of common plots using the `ggplot2` package. Here, you'll learn how to make and interpret the following types of plots, including: scatterplot, boxplot, bar graph, and histogram.

First, let's load the package 'gglpot2'.

```{r setup, warning=FALSE, results=FALSE, message=FALSE}

#the library function loads the ggplot2 package
library(ggplot2)
...
ggplot(merged_data, aes(x=log(DENV_MFI))) +
  ...
  ggtitle("Histogram of age (years)")
